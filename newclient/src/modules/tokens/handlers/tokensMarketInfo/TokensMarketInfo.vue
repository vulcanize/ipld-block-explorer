...
import { getLatestPrices_getLatestPrices as TokenMarketData, getAllContracts_getAllContracts as ContractsType  } from '@app/core/components/mixins/CoinData/apolloTypes/getLatestPrices'
...
export default class AddressTokens extends Mixins(CoinData) {
    ...
    tokensData: ContractsType[] | null = null
    allContracts: TokenMarketData[] | any[ any ]
    ...
    get showTokens(): ContractsType[] {
        console.log('showTokens', this)
        console.log('allContracts', this.allContracts)
        const start = this.index * this.maxItems
        if (!this.initialLoad && this.allContracts.nodes.length) {
            // const end = start + this.maxItems > this.tokensData.length ? this.tokensData.length : start + this.maxItems
            return this.allContracts.nodes
        }
        if (this.allContracts.nodes.length) {
    ...
    onisLoadingTokensMarketData(newVal: boolean, oldVal: boolean) {
        console.log('isLoadingTokensMarketData newVal ', newVal)
        // if (!newVal) {
