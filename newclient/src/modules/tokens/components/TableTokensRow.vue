...
<!--                    </v-flex>-->
                    ...
                    <v-flex xs8 sm9>
                        ...
                        <v-layout v-else row pa-2>
                            ...
                            <app-transform-hash :hash="token.contractId" :link="tokenLink" />
                        </v-layout>
                    </v-flex>
                    <v-flex xs2>
                        <v-layout row align-center justify-end>
                            <fav-handler-heart-actions :symbol="token.name" :address="token.contractId" :is-small="true" />
                        </v-layout>
                    </v-flex>
                    <v-flex xs12>
...
<!--                            <app-tooltip v-if="tokenPrice.tooltipText" :text="tokenPrice.tooltipText" />-->
                    ...
                    <v-flex md2>
                        <v-layout grid-list-xs row align-center justify-start>
                            <p :class="priceChangeClass">{{ token.contractId }}</p>
...
import { getAllContractsData_getAllContractsData as ContractsDataType } from '@app/core/components/mixins/CoinData/apolloTypes/getLatestPrices'
...
export default class TokenTableRow extends Mixins(NumberFormatMixin) {
    @Prop(Object) token!: ContractsDataType
    imageExists = true
    ...
    get tokenLink(): string {
        return `/token/${this.token.contractId}`
    }

    get tokenPrice(): FormattedNumber {
        const price = this.token.contractId || 0
        return this.formatUsdValue(new BN(price))
    }
    get percentageChange(): FormattedNumber {
        const change = this.token.contractId || 0
    ...
    get tokenVolume(): FormattedNumber {
        const volume = this.token.contractId || 0
        return this.formatIntegerValue(new BN(volume))
    }

    get tokenMarket(): FormattedNumber {
        const marketCap = this.token.contractId || 0
    ...
    mounted() {
      console.log('TableTokens ', this.token)
    }
}
</script>
