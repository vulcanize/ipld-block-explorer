            ...
            <v-flex xs12 hidden-md-and-up>
                <div class="table-row-mobile">
                    <v-layout grid-list-xs row wrap align-center justify-start fill-height class="pt-3 pb-3 pr-4 pl-4">
                        <v-flex xs6 pa-1>
                            <router-link :to="`/block/number/${block.contractId}`" class="black--text font-weight-medium pb-1"
                                >{{ _block.contractId }}</router-link
                            >
                        </v-flex>
                        <v-flex xs6 pr-44>
                            <v-layout row justify-end>
                                <p class="black--text align-center pl-2">
                                  {{ _block.name }}
<!--                                    {{ _block.name }} {{ $tc('tx.name-short', 2) }}-->
                                    <app-tooltip v-if="_block.name > 0" :text="txTooltipText" />
                                </p>
                            </v-layout>
                        </v-flex>
                        <v-flex xs2 pa-1>
                            <p class="info--text psmall">{{ _block.name }}:</p>
                        </v-flex>
<!--                        <v-flex xs10 pa-1>-->
...
<!--                        </v-flex>-->
<!--                        <v-flex xs10 pa-1>-->
<!--                            <app-transform-hash :hash="_block.name | toChecksum" :italic="true" :link="`/address/${_block.miner}`" />-->
<!--                        </v-flex>-->
<!--                        <v-flex xs2 pa-1>-->
...
<!--                        </v-flex>-->
            ...
            <v-flex hidden-sm-and-down sm12>
                <v-layout grid-list-xs row wrap align-center justify-start fill-height pl-3 pr-2 pt-2 pb-1>
                    <v-flex sm1>
                        <router-link :to="`/block/number/${block.id}`" class="black--text pb-1">{{ _block.blockNumber }}</router-link>
                    </v-flex>
                    <v-flex sm5>
                        <v-layout row pb-2>
...
<!--                    </v-flex>-->
                    <v-flex sm3>
                        <p class="black--text align-center mb-0">
                            {{ _block.reward }}
<!--                            <app-tooltip v-if="_block.rname" :text="`${_block.name} ${$t('common.eth')}`" />-->
                        </p>
                    </v-flex>
                </v-layout>
...
export default class TableBlocksRow extends Mixins(NumberFormatMixin) {
    ...
    mounted() {
      console.log('TableBlocksRow', this.block)
    }
}
</script>
